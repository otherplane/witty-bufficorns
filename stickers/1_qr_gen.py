...
def generate_badges(badges_count: int, salt: str, key_len: int):
    return [generate_badge(index, salt, key_len) for index in range(badges_count)]
...
def generate_qr_code(badge: Badge, base_url: str, output_dir: str):
    url = f'{base_url}{badge.key}'
    output_path = f'{output_dir}/{badge.index}-{badge.key}-{colorize(badge.index)}.png'
...
def generate_qr_codes(badges: List[Badge], base_url: str, output_dir: str):
    output_dir_fd = os.path.dirname(output_dir)
    if not os.path.exists(output_dir_fd):
        os.makedirs(output_dir_fd)

    return [generate_qr_code(badge, base_url, output_dir) for badge in badges]


def main(config):
    badges = generate_badges(int(config.badges), config.salt, int(config.key_len))
    [print(f'{badge.index} → {badge.key} ({colorize(badge.index)})') for badge in badges]
    generate_qr_codes(badges, config.base_url, config.output_dir)
